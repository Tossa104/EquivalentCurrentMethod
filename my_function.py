...
# 変数の位置変えてもp_x,p_y,p_zの位置が変わらない

def F_p( u, v, w, a, b, c, p : np.ndarray):
    # print("function_F")
...
def F_m( u, v, w, a, b, c, p : np.ndarray):
    # print("function_F")
    p_x, p_y, p_z = p[0], p[1], p[2]
    r_1p = ((u - (p_x - a))**2 + (v - (p_y + b))**2 + (w - (p_z - c))**2)**0.5
    r_2p = ((u - (p_x - a))**2 + (v - (p_y + b))**2 + (w - (p_z + c))**2)**0.5
...
def G_a_( u, v, w, a, b, c, p : np.ndarray):
    # print("function_G_")
    p_x, p_y, p_z = p[0], p[1], p[2]
    r_1p = ((u - (p_x + a))**2 + (v - (p_y + b))**2 + (w - (p_z - c))**2)**0.5
    r_2p = ((u - (p_x + a))**2 + (v - (p_y + b))**2 + (w - (p_z + c))**2)**0.5
    ...
    z_3p = w - (p_z + c)
    z_4p = w - (p_z - c)
    if abs(v - p_y) < b and abs(w - p_z) < c:
    # if abs(v) < b and abs(w) < c:
        G_ = 2*math.pi - abs(math.atan2(x_1p*r_1p, y_1p*z_1p) + math.atan2(x_2p*r_2p, y_2p*z_2p) + math.atan2(x_3p*r_3p, y_3p*z_3p) + math.atan2(x_4p*r_4p, y_4p*z_4p))
        # G_ = 2*math.pi - abs(math.atan2( y_1p*z_1p, x_1p*r_1p) + math.atan2( y_2p*z_2p, x_2p*r_2p) + math.atan2( y_3p*z_3p, x_3p*r_3p) + math.atan2( y_4p*z_4p, x_4p*r_4p))
    else:
        G_ = math.atan2(x_1p*r_1p, y_1p*z_1p) + math.atan2(x_2p*r_2p, y_2p*z_2p) + math.atan2(x_3p*r_3p, y_3p*z_3p) + math.atan2(x_4p*r_4p, y_4p*z_4p)
        # G_ = math.atan2( y_1p*z_1p, x_1p*r_1p) + math.atan2( y_2p*z_2p, x_2p*r_2p) + math.atan2( y_3p*z_3p, x_3p*r_3p) + math.atan2( y_4p*z_4p, x_4p*r_4p)
    return G_



def G_b_( u, v, w, a, b, c, p : np.ndarray):
    # print("function_G_")
    p_x, p_y, p_z = p[0], p[1], p[2]
    r_1p = ((u - (p_x - a))**2 + (v - (p_y + b))**2 + (w - (p_z - c))**2)**0.5
    r_2p = ((u - (p_x - a))**2 + (v - (p_y + b))**2 + (w - (p_z + c))**2)**0.5
...
def G_c_( u, v, w, a, b, c, p : np.ndarray):
    # print("function_G_")
    p_x, p_y, p_z = p[0], p[1], p[2]
    # print((u - (p_x - a))**2)
    # print((v - (p_y + b))**2)
    # print((w - (p_z + c))**2)
    # print(((u - (p_x - a))**2 + (v - (p_y + b))**2 + (w - (p_z + c))**2)**0.5)
    r_1p = ((u - (p_x - a))**2 + (v - (p_y + b))**2 + (w - (p_z + c))**2)**0.5
    r_2p = ((u - (p_x + a))**2 + (v - (p_y + b))**2 + (w - (p_z + c))**2)**0.5
    r_3p = ((u - (p_x + a))**2 + (v - (p_y + b))**2 + (w - (p_z - c))**2)**0.5
    r_4p = ((u - (p_x - a))**2 + (v - (p_y + b))**2 + (w - (p_z - c))**2)**0.5
    ...
    z_3p = w - (p_z - c)
    z_4p = w - (p_z - c)
    # print(math.atan2(y_1p*r_1p, x_1p*z_1p))
    # print(math.atan2(y_2p*r_2p, x_2p*z_2p))
    # print(math.atan2(y_3p*r_3p, x_3p*z_3p))
    # print(math.atan2(y_4p*r_4p, x_4p*z_4p))
    if abs(u - p_x) < a and abs(w - p_z) < c:
    # if abs(u)< a and abs(w) < c:
        G_ = 2*math.pi - abs(math.atan2(y_1p*r_1p, x_1p*z_1p) + math.atan2(y_2p*r_2p, x_2p*z_2p) + math.atan2(y_3p*r_3p, x_3p*z_3p) + math.atan2(y_4p*r_4p, x_4p*z_4p))
        # G_ = 2*math.pi - abs(math.atan2( x_1p*z_1p, y_1p*r_1p) + math.atan2( x_2p*z_2p, y_2p*r_2p) + math.atan2( x_3p*z_3p, y_3p*r_3p) + math.atan2( x_4p*z_4p, y_4p*r_4p))
    else:
        G_ = math.atan2(y_1p*r_1p, x_1p*z_1p) + math.atan2(y_2p*r_2p, x_2p*z_2p) + math.atan2(y_3p*r_3p, x_3p*z_3p) + math.atan2(y_4p*r_4p, x_4p*z_4p)
        # G_ = math.atan2( x_1p*z_1p, y_1p*r_1p) + math.atan2( x_2p*z_2p, y_1p*r_1p) + math.atan2( x_3p*z_3p, y_3p*r_3p) + math.atan2( x_4p*z_4p, y_4p*r_4p)
    # print(G_)
    return G_



def G_d_( u, v, w, a, b, c, p : np.ndarray):
    # print("function_G_")
    p_x, p_y, p_z = p[0], p[1], p[2]
    r_1p = ((u - (p_x - a))**2 + (v - (p_y - b))**2 + (w - (p_z + c))**2)**0.5
    r_2p = ((u - (p_x + a))**2 + (v - (p_y - b))**2 + (w - (p_z + c))**2)**0.5
...
def G_e_( u, v, w, a, b, c, p : np.ndarray):
    # print("function_G_")
    p_x, p_y, p_z = p[0], p[1], p[2]
    r_1p = ((u - (p_x + a))**2 + (v - (p_y - b))**2 + (w - (p_z + c))**2)**0.5
    r_2p = ((u - (p_x + a))**2 + (v - (p_y + b))**2 + (w - (p_z + c))**2)**0.5
    ...
    z_3p = w - (p_z + c)
    z_4p = w - (p_z + c)
    if abs(u - p_x) < a and abs(v - p_y) < b:
    # if abs(u) < a and abs(v) < b:
        G_ = 2*math.pi - abs(math.atan2(z_1p*r_1p, x_1p*y_1p) + math.atan2(z_2p*r_2p, x_2p*y_2p) + math.atan2(z_3p*r_3p, x_3p*y_3p) + math.atan2(z_4p*r_4p, x_4p*y_4p))
        # G_ = 2*math.pi - abs(math.atan2( x_1p*y_1p, z_1p*r_1p) + math.atan2( x_2p*y_2p, z_2p*r_2p) + math.atan2( x_3p*y_3p, z_3p*r_3p) + math.atan2( x_4p*y_4p, z_4p*r_4p))
    else:
        G_ = math.atan2(z_1p*r_1p, x_1p*y_1p) + math.atan2(z_2p*r_2p, x_2p*y_2p) + math.atan2(z_3p*r_3p, x_3p*y_3p) + math.atan2(z_4p*r_4p, x_4p*y_4p)
        # G_ = math.atan2( x_1p*y_1p, z_1p*r_1p) + math.atan2( x_2p*y_2p, z_2p*r_2p) + math.atan2( x_3p*y_3p, z_3p*r_3p) + math.atan2( x_4p*y_4p, z_4p*r_4p)
    return G_



def G_f_( u, v, w, a, b, c, p : np.ndarray):
    # print("function_G_")
    p_x, p_y, p_z = p[0], p[1], p[2]
    r_1p = ((u - (p_x + a))**2 + (v - (p_y - b))**2 + (w - (p_z - c))**2)**0.5
    r_2p = ((u - (p_x + a))**2 + (v - (p_y + b))**2 + (w - (p_z - c))**2)**0.5
...
def G_a( u, v, w, a, b, c, p : np.ndarray):
    ...
    x_3p = u - (p_x + a)
    x_4p = u - (p_x + a)
    G = -Sign(x_1p)*abs(G_a_( u, v, w, a, b, c, p)) -Sign(x_2p)*abs(G_a_( u, v, w, a, b, c, p)) -Sign(x_3p)*abs(G_a_( u, v, w, a, b, c, p)) -Sign(x_4p)*abs(G_a_( u, v, w, a, b, c, p))
    return G
...
def G_b( u, v, w, a, b, c, p : np.ndarray):
    ...
    x_3p = u - (p_x - a)
    x_4p = u - (p_x - a)
    G = -Sign(x_1p)*abs(G_b_( u, v, w, a, b, c, p)) -Sign(x_2p)*abs(G_b_( u, v, w, a, b, c, p)) -Sign(x_3p)*abs(G_b_( u, v, w, a, b, c, p)) -Sign(x_4p)*abs(G_b_( u, v, w, a, b, c, p))
    return G
...
def G_c( u, v, w, a, b, c, p : np.ndarray):
    ...
    y_3p = v - (p_y + b)
    y_4p = v - (p_y + b)
    G = -Sign(y_1p)*abs(G_c_( u, v, w, a, b, c, p)) -Sign(y_2p)*abs(G_c_( u, v, w, a, b, c, p)) -Sign(y_3p)*abs(G_c_( u, v, w, a, b, c, p)) -Sign(y_4p)*abs(G_c_( u, v, w, a, b, c, p))
    return G
...
def G_d( u, v, w, a, b, c, p : np.ndarray):
    ...
    y_3p = v - (p_y - b)
    y_4p = v - (p_y - b)
    G = -Sign(y_1p)*abs(G_d_( u, v, w, a, b, c, p)) -Sign(y_2p)*abs(G_d_( u, v, w, a, b, c, p)) -Sign(y_3p)*abs(G_d_( u, v, w, a, b, c, p)) -Sign(y_4p)*abs(G_d_( u, v, w, a, b, c, p))
    return G
...
def G_e( u, v, w, a, b, c, p : np.ndarray):
    ...
    z_3p = w - (p_z + c)
    z_4p = w - (p_z + c)
    G = -Sign(z_1p)*abs(G_e_( u, v, w, a, b, c, p)) -Sign(z_2p)*abs(G_e_( u, v, w, a, b, c, p)) -Sign(z_3p)*abs(G_e_( u, v, w, a, b, c, p)) -Sign(z_4p)*abs(G_e_( u, v, w, a, b, c, p))
    return G
...
def function_alpha_x(j_cen : np.ndarray, a, b, c, i_cen : np.ndarray):
    ...
    alph_x = G_c( u, v, w, a, b, c, j_cen) - G_d( u, v, w, a, b, c, j_cen) + G_e( u, v, w, a, b, c, j_cen) - G_f( u, v, w, a, b, c, j_cen)
    return alph_x
...
def function_beta_y(j_cen : np.ndarray, a, b, c, i_cen : np.ndarray):
    u,v,w = i_cen[0], i_cen[1], i_cen[2]
    beta_y = G_a( u, v, w ,a, b, c, j_cen) - G_b( u, v, w ,a, b, c, j_cen) + G_e(u, v, w, a, b, c, j_cen) - G_f(u, v, w, a, b, c, j_cen)
    return beta_y
...
def function_gamma_z(j_cen : np.ndarray, a, b, c, i_cen : np.ndarray):
    u,v,w = i_cen[0], i_cen[1], i_cen[2]
    gam_z = G_a( u, v, w ,a, b, c, j_cen) - G_b( u, v, w ,a, b, c, j_cen) + G_c( u, v, w, a, b, c, j_cen) - G_d( u, v, w, a, b, c, j_cen)
    return gam_z

# 磁性体要素の中心の配列(n×3)を渡すと磁化ベクトルの係数行列(3n×3n)を返す
def function_M(cens : np.ndarray, a, b, c):
    n = cens.shape[0]
    print(n)
    ...
    for i in range(n):
        ...
        for j in range(n):
            ...
            # print(j_cen)
            alph_x = function_alpha_x( j_cen, a, b, c, i_cen)
        ...
        for k in range(n):
            ...
            beta_y = function_beta_y(k_cen, a, b, c, i_cen)
        ...
        for l in range(n):
            ...
            gamma_z = function_gamma_z( l_cen, a, b, c, i_cen)
...
def function_J( m : np.ndarray, Bc : np.ndarray, ur):
    ...
    # np.savetxt('M.csv' ,((1-ur)/(4*np.pi))*m , delimiter=",")
...
# u : 透磁率
def J_solver( cen : np.ndarray, w, d, h, n, m, l, r, i, u):
    ...
    print("Bc" ,Bc)
    M = function_M(cens_list_np, a, b, c)
    # print("M " ,M)
    J = function_J(M, Bc, u)
...
def function_Mm( cens : np.ndarray, a, b, c, p : np.ndarray):
    n = cens.shape[0]
    print(n)
    cens_list = cens.tolist()
    m_list = []
    for j in range(n):
        j_cen = np.array(cens_list[j])
        alph_x = function_alpha_x( j_cen, a, b, c, p)
    ...
    for k in range(n):
        ...
        beta_y = function_beta_y(k_cen, a, b, c, p)
    ...
    for l in range(n):
        ...
        gamma_z = function_gamma_z( l_cen, a, b, c, p)
        m_list.append(alph_z)
        m_list.append(beta_z)
        m_list.append(gamma_z)
    m_np = np.array(m_list)
...
def B_solver( cen : np.ndarray, w, d, h, n, m, l, r, i, u, p : np.array):
    cens_list_np, a, b, c = make_elements(cen, w, d, h, n, m, l)
    ...
    Mm = function_Mm(cens_list_np, a, b, c, p)
